...
本 change 只拥有 `shared/promptrepo` 内的 workflow、质量命令、项目文档和验证，不把其他仓库作为完成门禁。
...
- Profile: `public pure-Go library`.
- quick commands: `go mod verify`, `go test ./...`, `go vet ./...`, `CGO_ENABLED=0 go build ./...`
- full commands: `CGO_ENABLED=0 go test ./...`, `go test ./... -run Compatibility`
- integration commands: 不适用
- release: Tags and public module publication remain explicit maintainer actions.
